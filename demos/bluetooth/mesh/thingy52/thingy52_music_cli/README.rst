...
By pushing the button on the Thingy:52, the on-board microphone starts sapling incomming sound. Pushing the button again deactivates the sampling.
...
The Thingy:52 microphone outputs PDM data that must be processed. To do this an :ref:`FFT analyzer module <thingy52_fft_analyzer>` located in the ``common/fft_analyzer`` folder is used. 
The analyzer inputs raw PDM data, and outputs a discrete spectrum array. Each element of the array represent one frequency, and the value of the element is amplitude.
As default, an FFT buffer size of 512 is used. This generates a discrete spectrum array of 256 (half of the FFT buffer size). 
To find what frequency each element of the spectrum array represent, use this formula:

.. math::

   f_{n} = \frac{\text{sampling frequency}}{\text{FFT buffer size}} * n
...
When Button 0 (the only button) on the Thingy:52 is pressed, the on-board microphone starts sapling incomming sound. The main LED lights up when the microphone is sampling.
